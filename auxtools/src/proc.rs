...
static mut PROCS_BY_NAME: Option<HashMap<String, Vec<Proc>, RandomState>> = None;
static mut PROC_OVERRIDE_IDS: Option<HashMap<raw_types::procs::ProcId, u32, RandomState>> = None;
...
pub fn populate_procs() {
	let mut i: u32 = 0;
	let override_ids =
		unsafe { PROC_OVERRIDE_IDS.get_or_insert_with(|| HashMap::with_hasher(RandomState::default())) };
	let h = unsafe { PROCS_BY_NAME .get_or_insert_with(|| HashMap::with_hasher(RandomState::default())) };
	loop {
